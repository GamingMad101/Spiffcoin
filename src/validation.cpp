...
#if defined(NDEBUG)
# error "spiffcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "spiffcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetspiffcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // spiffcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("spiffcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // spiffcoin: BIP30 has been active since inception
    ...
    // spiffcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetspiffcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // spiffcoin: Disallow AuxPow blocks before it is activated.
    ...
    // spiffcoin: Version 2 enforcement was never used
    ...
    // spiffcoin: Introduce supermajority rules for v4 blocks
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // spiffcoin: We probably want to disable this
    int nLockTimeFlags = 0;
